...
# ==========================================
# 🔍 이미지 확대 보기 팝업 함수 (모바일 터치 최적화)
# ==========================================
@st.dialog("🔍 이미지 확대 보기")
def zoom_image(img_source):
...
# 💡 기준정보 팝업 기능 (확장자 스마트 자동 감지 로직)
with st.expander("📊 GRE PIPE 검사 기준표 보기 (클릭하여 펼치기)"):
    image_found = False
    # png, jpg, jpeg 대소문자 가리지 않고 모두 뒤져서 찾아냅니다!
    for ext in ['png', 'jpg', 'jpeg', 'PNG', 'JPG', 'JPEG']:
        file_name = f"reference.{ext}"
        if os.path.exists(file_name):
            st.image(file_name, caption="GRE PIPE 기준 데이터", use_container_width=True)
            
            # 🟢 기준표 확대 버튼 추가
            if st.button("🔍 기준표 확대 보기", key="btn_zoom_ref"):
                zoom_image(file_name)
                
            image_found = True
            break
            
    if not image_found:
        st.info("💡 깃허브에 'reference.png' 또는 'reference.jpg' 파일을 올려주시면 이곳에 표시됩니다!")
...
with col_p1:
    ...
    if up1:
        ...
        # 🟢 확대 버튼 추가
        if st.button("🔍 확대 보기", key="btn_zoom1", use_container_width=True):
            zoom_image(st.session_state['photo1'])
    else:
...
with col_p2:
    ...
    if up2:
        ...
        # 🟢 확대 버튼 추가
        if st.button("🔍 확대 보기", key="btn_zoom2", use_container_width=True):
            zoom_image(st.session_state['photo2'])
    else:
...
with col_p3:
    ...
    if up3:
        ...
        # 🟢 확대 버튼 추가
        if st.button("🔍 확대 보기", key="btn_zoom3", use_container_width=True):
            zoom_image(st.session_state['photo3'])
    else:
...
with col_p4:
    ...
    if up4:
        ...
        # 🟢 확대 버튼 추가
        if st.button("🔍 확대 보기", key="btn_zoom4", use_container_width=True):
            zoom_image(st.session_state['photo4'])
    else:
